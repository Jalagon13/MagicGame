...
public class SaveSystem : MonoBehaviour
{
    ...
    {
        _path = Application.dataPath + $"/_CraftGame/Configuration/JsonData/{WorldManager.Instance.GetActiveEnvironmentID()}_data.json";
        ...
        foreach (var chunkPosChunkDataKVP in ChunkManager.Instance.GetChunksFromActiveEnvironment())
        {
            ...
            if(worldAssetGameDataList.Count > 0)
            {
                foreach (var worldAssetGameData in worldAssetGameDataList)
                {
                    ...
                    {
                        // Fetch and convert position
                        Vector2Int posToSend = new Vector2Int
                        (
                            Mathf.FloorToInt(worldAssetGameData.Asset.transform.position.x),
                            Mathf.FloorToInt(worldAssetGameData.Asset.transform.position.y)
                        );
				
                        // Create new filedata for this asset
                        WorldAssetFileData worldAssetData = new()
                        {
                            WorldAssetId = GameManager.Instance.GetByteIDFromWorldAsset(worldAssetGameData.Asset),
                            Pos = posToSend
                        };
    ...
    public async Task DeserializeAndDispatchData()
    {
        ...
        if (File.Exists(_path))
        {
            ...
            Debug.Log($"<color=orange>All Data of: </color>{WorldManager.Instance.GetActiveEnvironmentID()}<color=orange> Deserialized And Dispatched! </color>");
        }
        else
        {
    ...
    private void DeserializeAssetData()
    {
        ...
        Debug.Log($"<color=orange>Asset Data of: </color>{WorldManager.Instance.GetActiveEnvironmentID()}<color=orange> Deserialized And Instantiated </color>");
    }
}
