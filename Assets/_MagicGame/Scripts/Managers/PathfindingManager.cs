...
public class PathfindingManager : NetworkBehaviour
{
	...
	public void UpdatePathfinding(Vector2Int chunkPos, ChunkGameData chunkGameData, EnvironmentID environment)
	{
		Debug.Log($"Updating pathfinding for chunk {chunkPos}");
		InvisibleTilemapAddChunk(chunkPos, chunkGameData);
		UpdateAstarGrid(chunkPos);
	}
	
	private void InvisibleTilemapAddChunk(Vector2Int chunkPos, ChunkGameData chunkGameData)
	{
