...
public class TilemapData : NetworkBehaviour
{
	...
	private void AddTileToNetworkListDamaged(Vector2Int position, byte tileId, ushort damageAmount, EnvironmentID environment)
	{
		...
		if(currentTileHpAfterDamage > 0)
		{
			...
			{
				TileID = GameManager.Instance.GetTileIdFromTileSO(tileSO),
	...
	{
		return GameManager.Instance.GetTileSOFromID(GetSyncTileHpDataFromPosition(position).TileID).HarvestType;
	}
