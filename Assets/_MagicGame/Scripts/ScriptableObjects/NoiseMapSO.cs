...
{
    public int Width = 256; // Width of the generated noise map in pixels.
    public int Height = 256; // Height of the generated noise map in pixels.
    public float Scale = 20f; // Scale of the noise map. A lower scale value will zoom in on the noise, resulting in larger, smoother regions.
    [Range(1, 10)]
    public int Octaves = 1; // Number of octaves used for the noise generation, controlling the detail level.
    [Range(0.01f, 1f)]
    public float Persistence = 0.5f; // Persistence controls the amplitude of each octave, affecting the roughness of the noise.
    [Range(1f, 3.5f)]
    public float Lacunarity = 2f; // Lacunarity controls the frequency of each octave, influencing the frequency of the noise.
