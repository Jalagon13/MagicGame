    ...
    private IEnumerator SpellLifetimeRoutine()
    {
        ...
        if (SpellData.Value.HoldToCast)
        {
            ...
            while (SpellStateNV.Value == SpellState.Casting)
            {
                ...
                // Drain Essence Cost if player is casting this
                if(SpellCasterNetworkObject.gameObject.transform.TryGetComponent(out Player player))
                {
                    player.SpellCastController.TryToConsumeCastingMaterials();
                }
            }
        }
        else
